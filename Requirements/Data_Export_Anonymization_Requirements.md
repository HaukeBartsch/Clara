...
| ID | Requirement |
|---|---|
| REQ-EXP-020 | A **direct identifier field** MUST be classified as: the record identifier field (GD-8), or a field whose validation type is `email` or `MRN` (REQ-VAL-015…039). Direct identifier fields MUST be removed — the column absent from the output — at both `export_de_identified` and `export_no_identifiers` (D-1). |
| REQ-EXP-021 | **Fail-safe:** an `email`/`MRN` value MUST NOT leave the system untransformed at any level below `export_full`, regardless of the field's personal-information flag (D-1). |
