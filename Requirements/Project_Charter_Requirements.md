...
| BR-012 | User accounts MUST support a validity period (days; `0` = indefinite) and MUST be auto-disabled after a configurable period of inactivity (default 180 days), re-enableable by an administrator (GD-19). |
| BR-013 | Every project MUST be in exactly one of three modes — development, production, analysis: staged setup changes with a breaking-change warning in production, disabled data entry in analysis, and admin-only transitions (GD-20). |

## 4. Scope — Phase 1

**In scope:**
- User account management (enable/disable, admin flag, account validity period, inactivity auto-disable with admin re-enable — GD-19).
- Table-based (local password) authentication: a third login path beside OAuth2 and LDAP, available from first installation (GD-18).
...
- Project modes (development / production / analysis): staged setup changes in production with a breaking-change warning before commit, disabled data entry in analysis, admin-only transitions (GD-20).
...
| GD-4 | **Admin status:** a boolean `is_admin` on the users table. The bootstrap admin is defined by `ADMIN_BOOTSTRAP_EMAIL` in configuration; the API promotes a matching login on first occurrence. Admins are managed through the users API. (Extended 2026-09-22 by GD-18: on a first install without an identity provider, the bootstrap admin's local password is set from `ADMIN_BOOTSTRAP_PASSWORD`.) |
...
| GD-7 | **Time:** all **system** timestamps (audit entries, account and project metadata, API-level response timestamps) are stored and compared in UTC. (Scope clarified 2026-09-22 by GD-16: clinical date/date-time values carry their collection timezone.) |
...
| GD-14 | **Data-entry submission policy (2026-09-22):** fields may have no value; an instrument that is partially filled and saved stores only the entered values. An empty value supplied to the API is a value — it intentionally clears the previously stored value — so the **web application MUST NOT submit empty values for fields whose previous value the user has not removed**: it submits exactly the fields that carry a value in the form plus the fields the user explicitly cleared. The API import semantics (empty = "no value"/clear, REQ-VAL-024) are unchanged; this is a UI-side decision. |
| GD-15 | **Event ordering (2026-09-22):** within an arm, events **with** a timepoint value (`period`, days after baseline) are sorted by that timepoint (ascending; ties by user position); events **without** a timepoint value (`period` = `NULL`) are user-orderable through the events table (a new events-order endpoint). `events.period` is therefore nullable: `NULL` = no timepoint. The canonical per-arm order applies to the UI event table, the record-status dashboard, `content=event`, and `content=formEventMapping`. |
| GD-16 | **Timezones in the internal date/date-time format (2026-09-22):** the canonical storage form of date and date-time values carries the timezone of collection: dates `YYYY-MM-DD±HH:MM`, date-times `YYYY-MM-DD HH:MM±HH:MM` (offset `±HH:MM`, UTC = `+00:00`). The offset for an imported value comes, in order, from (1) the zone supplied with the import — the browser timezone for UI data entry, an optional `tz` parameter for data-API imports — and (2) the deployment default `APP_TIMEZONE` (configuration; default `UTC`). Values are stored as collected (wall time + offset), never converted; GD-7 continues to govern system timestamps only. Anonymized-export date shifting applies to the date part and preserves the offset; chronological comparisons use absolute instants. |
| GD-17 | **Simplified `projects` table (2026-09-22):** the `projects` table stores project name, organization (the main supporting institution), PI name/email, data-manager name/email, REK number, REK start/end dates, start/end dates, participant naming pattern, and creation time. The option flags (radiology, pathology, pathology type, redcap-only, data collection from home), the end-user-contract flag, the end provision, and the initial `event_names` list are **removed** from the table. If the owner wants them, they are stored as data in an ordinary instrument of the project (master spec: `DataTransferProjects`) — the system gives that instrument no special handling. Project creation no longer derives initial events; events are added through the events endpoint. |
| GD-18 | **Table-based authentication (2026-09-22):** the `users` table gains a nullable `password_hash` column (bcrypt) and `auth_source` gains `local`. An email+password login path (PHP form → API verification of the stored hash) exists beside the OAuth2 and LDAP paths, and is tried **first** on the password form (no external dependency), with LDAP as the remaining fallback. On a first installation without an OAuth2 provider or LDAP server, the bootstrap admin (`ADMIN_BOOTSTRAP_EMAIL`) receives its local password from `ADMIN_BOOTSTRAP_PASSWORD` (required at startup in that case), so the system is usable and its authentication/authorization can be set up through the existing administration UI. Plaintext passwords are never stored or logged (REQ-AUTH-036 revised). |
| GD-19 | **Account validity and inactivity (2026-09-22):** the `users` table gains `valid_until` (date; `NULL` = indefinite; set as days, `0` = indefinite) and `last_login_at` (UTC; updated on every successful login). An account is active only while `enabled`, not expired (`valid_until`), and not inactive: if the last successful login is older than `AUTH_INACTIVITY_LIMIT_DAYS` (configuration; default 180; `0` = rule off), the account is auto-disabled at the check, the inactivity clock is reset, the attempt is rejected, and the event is audit-logged (`account_auto_disabled`). An administrator re-enables such accounts (user overview shows last login, validity end, and status). |
| GD-20 | **Project modes (2026-09-25):** every project is in exactly one of three modes — `development` (default for new projects), `production`, `analysis`. Only a `project_admin` may change the mode. **Development:** setup and data entry work as usual; all functionality available. **Production:** setup changes are staged and activated as a group — *start staging* opens a staging set (e.g., add or modify instruments); while staged, ongoing data collection continues to use the currently active design; *commit staged changes* activates the whole set at once. Before committing, warn about breaking changes — changes that would make existing project data inconsistent/inaccessible (e.g., deleting a field); non-breaking changes (adding a field, changing a field description, adding options to an existing dropdown) need no warning. **Analysis:** data entry is disabled; viewing and exporting remain available per each user's permissions; admin users can still interact with the project. **Transitions** (project admin): development → production asks whether previously stored data should be kept or deleted; production → development keeps all data; production ↔ analysis keeps all data. |
